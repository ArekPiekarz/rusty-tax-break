...
pub fn setupDispatching(handlers: EventHandlers, receiver: Receiver)
{
    ...
    attach(receiver, move |(source, event)| { match (source, &event) {
        ...
        (S::OutputPathStore,                    E::OutputPathChanged(_))          => (&mut outputPathLabel, &mut reportGenerator).handle(source, &event),
        (S::OutputPathStore,                    E::PartialOutputPathChanged(_))   => outputPathLabel.handle(source, &event),
        (S::RepositoryStore,                    E::RepositoryChanged(_))          => (&mut repositoryPathLabel, &mut commitLog, &mut commitDiffView, &mut reportGenerator).handle(source, &event),
